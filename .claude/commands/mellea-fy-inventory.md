...
- `VALIDATE_OUTPUT`: checks on what *the agent itself produced*. → `Requirement`.
- `VALIDATE_DOMAIN`: checks the agent *performs on external data* (code, papers, patches). → `m.instruct(format=VerdictSchema)`.
