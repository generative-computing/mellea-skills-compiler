...
`examples/weather/weather_mellea/fixtures/`. It feeds the pipeline the
...
```bash
mellea-skills run examples/sentry-find-bugs/sentry_find_bugs_mellea --fixture clean_secure_parameterized
```
...
```bash
mellea-skills run examples/superpowers-systematic-debugging/superpowers_systematic_debugging_mellea --fixture architectural_issue_detected
```
...
>
> ```bash
> chmod +x examples/clawdefender/clawdefender_mellea/scripts/*.sh
> ```
